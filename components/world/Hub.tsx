...
import { AdditiveBlending, CanvasTexture, type Group, type PointLight, type Sprite, type SpriteMaterial } from "three";
...
type Props = {
  ...
  haloScale?: number;
  /** attenua l'hub quando sei dentro un sotto-layer (es. i progetti) */
  dim?: boolean;
};
 ...
 */
export function Hub({ reduced, pulseKey, haloScale = 1, dim = false }: Props) {
  ...
  useFrame((_, dt) => {
    ...
    // attenuazione morbida quando sei in un sotto-layer
    dimF.current += ((dim ? 0.28 : 1) - dimF.current) * Math.min(1, dt * 4);
    const df = dimF.current;
    if (light.current) light.current.intensity = 12 * df;
    if (root.current) root.current.scale.setScalar(intro * (0.55 + 0.45 * df) * (1 + 0.22 * p));
    if (halo.current) {
      halo.current.scale.setScalar(HALO * haloScale * (1 + 0.35 * p));
      (halo.current.material as SpriteMaterial).opacity = (0.32 + 0.3 * p) * df;
    }
  ...
  return (
    <group ref={root}>
      <pointLight ref={light} color={P.accent} intensity={12} distance={16} decay={2} />
      <group ref={spin}>
